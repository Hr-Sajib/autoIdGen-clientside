...
export function EditCard({
  ...
  const handleFileChange = (file: File | null) => {
    ...
    if (file.size > MAX_FILE_SIZE) {
      ...
      toast.error("File size too large", {
        description: `Image size is ${(file.size / 1024 / 1024).toFixed(2)}MB. Maximum allowed is 10MB`
      })
      return
    }
  ...
  const handleCropSave = async () => {
    ...
    try {
      setIsCropping(true)
      console.log("Starting crop process...", croppedAreaPixels)
      toast.loading("Cropping image...", { id: "crop" })
      ...
      formDataAPI.append('file', blob, 'profile-image.jpg')
      formDataAPI.append('background_color', personPhotoBGColorCode || "#ffffff")
      formDataAPI.append('width', "200")
      formDataAPI.append('height', "200")
      formDataAPI.append('enhance_quality', "true")
      formDataAPI.append('center_face', "false")
      ...
      if (!apiResponse.ok) {
        ...
        throw new Error(`Background removal failed (Status: ${apiResponse.status})`)
      }
      ...
      if (contentType && contentType.includes('application/json')) {
        ...
        processedImageBlob = await apiResponse.blob()
        const processedImageUrl = URL.createObjectURL(processedImageBlob)
        console.log("Direct image response")
        setPhotoPreview(processedImageUrl)
      }
      ...
      if (processedImageBlob) {
        ...
        console.log("No processed image, using fallback")
      ...
      if (croppedImage) {
        ...
        try {
          ...
          if (hostedUrl) {
            ...
            handleChange("personalPhotoUrl", hostedUrl)
            setPhotoPreview(hostedUrl)
            toast.success("Image uploaded (background removal skipped)", { id: "crop" })
          } else {
  ...
  const handleTakePhoto = async () => {
    try {
      ...
      console.error("Webcam error:", err)
      console.log("Webcam failed, using file input")
  ...
  // REMOVED: Phone & Date validations
  const validateForm = (): boolean => {
    const newErrors: { [key: string]: string } = {}

    if (!formData.serialOrRollNumber || String(formData.serialOrRollNumber).trim() === "") {
      newErrors.serialOrRollNumber = "Serial/Roll Number is required"
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(formData._id ? "Card updated successfully" : "Card created successfully")
    } catch (error) {
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                readOnly
                value={formData.serialOrRollNumber || ""}
                onChange={(e) => handleChange("serialOrRollNumber", e.target.value)}
                className={`bg-gray-50 border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 rounded-lg transition-colors ${
                  errors.serialOrRollNumber ? "border-red-500" : ""
                }`}
                placeholder="Enter serial or roll number"
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex gap-2 flex-wrap">
                ...
                <button
                  ...
                  onClick={handleTakePhoto}
                  disabled={isAnyLoading}
                  className="px-4 py-2 rounded-lg border border-gray-300 hover:bg-gray-50 flex gap-2 items-center text-sm font-medium disabled:opacity-50"
                >
                  <IoCameraOutline size={16} />
                  <span>Take Photo</span>
                </button>
            ...
            {/* Dynamic Additional Fields - ALL TEXT */}
            {Object.entries(formData.additionalFields || {}).map(([prop, value], index) => {
              return (
                <div key={index} className="space-y-1.5">
                  <Label htmlFor={`field-${index}`} className="text-sm font-medium text-gray-700">
